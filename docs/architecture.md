...
- `scripts/`：CLI 层（`run_single.py`、`comsol_adapter.py`、`build_comsol_structure.py` 等）。
...
每个已接受结果目录含 `run_manifest.json`：配置 SHA、Git commit、环境版本、运行时长、输入/输出校验和。CLI 失败必须返回非零退出码（见 `scripts/run_single.py`）。
